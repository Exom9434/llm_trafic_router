...
def main() -> None:
    ...
    for model in sorted(summary):
        ...
        add(f"| {model} | " + " | ".join(cells) + " |")
    add("")
    add("*(칸은 은행 300으로 검정력 0.80을 유지하는 tau의 상한이다. 자기일관성만 본 값이다.)*")
    add("")

    # ── 7. 세 지표 합산 ──
    add("## 7. 세 지표를 함께 보면 규칙이 다르게 답한다")
    add("")
    add("7.5절의 규칙은 자기일관성 하나만 보고 쓰였다. 그런데 k는 자기일관성에만")
    add("걸리는 값이 아니다. **방문 1회가 답을 k개 뽑으므로 정확도의 조건당 관측")
    add("수도 방문 수 x k다.** k를 줄이면 자기일관성의 분산이 커지는 동시에")
    add("정확도의 관측이 줄어든다. 규칙을 세 지표로 다시 쓰면 어느 지표가 병목인지가")
    add("tau에 따라 바뀐다.")
    add("")
    add("정확도는 콜 하나가 0/1을 주므로 p(1−p)가 맞는 식이다. 기저 정확도는")
    add("설계서대로 0.60을 쓴다(p(1−p)가 최대인 0.5에 가까워 보수적이다).")
    add("추론 토큰의 변동계수는 보정 로그에서 쟀다 — DeepSeek 0.41, Qwen 0.28로")
    add("설계서가 가정한 0.6보다 작다. 아래는 큰 쪽인 0.41을 쓴다.")
    add("")
    add("| k | 정확도 관측 | 정확도 | 자기일관성 | 추론 토큰 | 세 지표 커버리지 |")
    add("|---:|---:|---|---|---|---:|")
    ACC_P, CV = 0.60, 0.41
    def n_acc(r, tau):
        return n_items_paired(0.03, sqrt(2 * ACC_P * (1 - ACC_P) / r + tau ** 2))
    def n_rt(r, tau):
        return n_items_paired(0.10, sqrt(2 * CV ** 2 / r + tau ** 2))
    def n_sc(k, tau):
        d = statistics.fmean(drops[m][k] for m in summary)
        return n_items_paired(d, sqrt(2 * var_k[k] / VISITS_PER_CONDITION + tau ** 2))
    for k in ks:
        r = VISITS_PER_CONDITION * k
        lo, hi = 0.0, 0.5
        for _ in range(40):
            mid = (lo + hi) / 2
            if max(n_acc(r, mid), n_sc(k, mid), n_rt(r, mid)) <= BANK_SIZE:
                lo = mid
            else:
                hi = mid
        add(f"| {k} | {r} | {n_acc(r, 0.15):,} | {n_sc(k, 0.15):,} | "
            f"{n_rt(r, 0.15):,} | **{lo:.3f}** |")
    add("")
    add("*(가운데 세 칸은 tau=0.15에서의 필요 문항 수, 마지막 칸은 세 지표가 모두")
    add("은행 300 안에 드는 tau의 상한이다.)*")
    add("")
    add("**병목이 자기일관성에서 정확도로 넘어간다.** tau=0.15에서 자기일관성은")
    add("204문항이면 되는데 정확도는 274문항을 요구한다. 6절이 자기일관성만 보고")
    add("0.18까지 간다고 한 것은 설계 전체의 커버리지가 아니다.")
    add("")
    add("이 표는 규칙의 방향도 뒤집는다. **선언할 tau를 먼저 고르고 최소 k를")
    add("고르면 순환에 빠진다.** 0.10을 선언하면 규칙은 k=2를 고르고, 0.15를")
    add("선언하면 k=3, 0.17이면 k=5를 고른다. 선언값이 k를 정하는 손잡이가 되는데,")
    add("그 선언값을 정할 독립적인 근거가 없다.")
    add("")
    add("**순서를 뒤집는 편이 정직하다.** 은행 300과 하루 8슬롯 14일은 이미 고정이다.")
    add("그러면 k마다 도달 가능한 커버리지가 하나씩 정해진다. k를 비용 대비")
    add("커버리지로 고르고, **선언값은 그 k가 도달한 값을 적는다.** 손잡이가 아니라")
    add("결과가 된다.")
    add("")
    ...
    add("## 8. 결론")
    add("")
    add("**첫째, k=3은 유지되지만 규칙은 다시 써야 한다.** 7.5절의 규칙은")
    add("자기일관성만 보고 tau 0.10까지 되는 최소 k라고 썼다. 그 문장을 실측값에")
    add("그대로 대면 답이 k=3이 아니라 k=2다(4절 표에서 138문항). 세 지표를 넣어도")
    add("tau=0.10에서는 k=2가 통과한다. 지금의 k=3은 규칙이 아니라 규칙 밖의 판단으로")
    add("서 있다.")
    add("")
    add("**둘째, 선언값을 먼저 고르는 형태가 순환이다.** 0.10을 선언하면 규칙이")
    add("k=2를, 0.15면 k=3을, 0.17이면 k=5를 고른다. 선언값이 k를 정하는 손잡이인데")
    add("그 값을 정할 독립적 근거가 없다. 은행 300과 일정이 이미 고정이므로 k마다")
    add("도달 가능한 커버리지가 하나씩 정해진다. k를 비용 대비 커버리지로 고르고")
    add("선언값은 그 결과를 적는 편이 낫다.")
    add("")
    add("| k | 세 지표 커버리지 | 비용(k=3 대비) |")
    add("|---:|---:|---:|")
    for k in ks:
        r = VISITS_PER_CONDITION * k
        lo, hi = 0.0, 0.5
        for _ in range(40):
            ...
            if max(n_acc(r, mid), n_sc(k, mid), n_rt(r, mid)) <= BANK_SIZE:
                ...
                hi = mid
        add(f"| {k} | {lo:.3f} | {k / 3:.2f}x |")
    add("")
    add("k=5는 커버리지를 0.01 넓히면서 비용을 1.67배로 만들고, k=2는 0.015를")
    add("잃으면서 3분의 1을 아낀다. k=3이 그 사이에서 가장 평평한 자리다.")
    add("")
    add("**셋째, 병목 지표가 자기일관성이 아니다.** tau=0.15에서 자기일관성은")
    add("205문항, 정확도는 274문항을 요구한다. 규칙을 자기일관성으로만 쓴 것이")
    add("애초의 실수였다. k는 자기일관성의 분산과 정확도의 관측 수를 동시에 움직인다.")
    add("")
    add("**넷째, 표집분산의 절대 수준이 크게 과대평가돼 있었다.** 옛 식은 방문")
    add("1회가 0/1을 준다고 보고 p(1−p)=0.128을 썼는데, 실측은 k=2에서 0.029,")
    add("k=5에서 0.011이다. 4배에서 12배 차이다. 그래서 자기일관성 쪽 필요 문항")
    add("수가 tau=0.10 기준 285에서 99로 내려간다.")
    add("")
    add("**다섯째, 배수의 가정값 하나는 틀렸다.** k=3의 1.60은 분산이 1/k로 준다고")
    add("본 값인데 실측은 1.36이다. k=2는 2.50 대 2.64로 반대 방향이다.")
    add("")
    add("**여섯째, 노이즈 바닥선을 k=3에서 다시 계산해야 한다.** 3절 표에서")
    add("E[c_3]이 E[c_5]보다 0.4~3.8%p 높다. 바닥선은 k=5로 쟀는데 본실험은 k=3으로")
    add("돈다. 검출 목표가 3%p인데 기준선이 최대 2.8%p 어긋나 있으면, 부하가 없어도")
    add("바닥선 아래로 떨어진 것처럼 보인다. 09-03에 잡은 풀 720 기준 바닥선 오류와")
    add("정확히 같은 종류다. 다만 다시 돌릴 필요는 없다. 5개 표본에서 3개씩")
    add("부분표집하면 편향 없이 나온다(3절 값이 그것이다).")
    add("")
    add("**일곱째, 추론 토큰의 변동계수는 가정보다 작다.** 설계서가 cv=0.6을")
    add("가정했는데 실측은 DeepSeek 0.41, Qwen 0.28이다. 이 지표는 어느 k에서도")
    add("20문항대면 충분하므로 규모를 정하는 데 관여하지 않는다.")
    add("")
    add("## 9. 한계")
    add("")
