...
**보정 패스는 이 둘 중 앞항만 잰다.** 노이즈 바닥선은 같은 문항을 반복했을
때의 흔들림, 즉 문항 내 표집분산이다. 반면 tau는 문항 i의 *조건 간 차이*
d_i가 문항마다 얼마나 다른가이므로 두 조건이 모두 있어야 계산된다. 보정
패스는 저부하 시간대에서만 돌기 때문에(설계서 6.1절) 고부하 관측이 없고,
따라서 tau를 추정할 수 없다.

이건 결함이 아니라 같은 성질의 양면이다. 보정 패스에 시간대 대비가 없다는
사실이 사전등록에서 "파일럿으로 결과를 엿볼 수 없었다"는 방어가 되고,
동시에 tau를 못 재게 만든다.

그러므로 tau는 본실험 전까지 가정값으로 남는다. 설계는 점추정 하나가 아니라
**tau 범위에 대한 민감도**로 정당화하고, 실현된 tau는 본실험 데이터에서
사후 보고한다.
