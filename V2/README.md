...
2. k 곡선 실측 — 7.5절 표의 분산 배수가 아직 짐작값이다
3. 08-27 TLS 사고의 원인 — 40분간 네 프로바이더가 동시에 죽고 실행이 멈췄다.
   14일 무인 실행 중에 같은 일이 나면 슬롯이 통째로 빈다

본실험 API 비용은 $161이고 예산 상한은 30만원($217)이다. 남는 $56은 재실행
몫이다. 결제 준비도 본실험 전에 끝내야 한다. DeepSeek 선불 잔액이 $7 안팎인데
본실험에 $73이 필요하고, Qwen은 상한을 $20으로 올려야 한다.
