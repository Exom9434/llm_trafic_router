...
본실험의 대비가 줄어든다. 러너는 현재 시각이 저부하 시간대를 벗어나면 실행을
...
| 구성 | 콜 수 | 추정 비용 |
|---|---:|---:|
| 후보 720문항, 라인업 7개 | 30,240 | 약 $4.0 |
| 후보 720문항, 앵커 포함 9개 | 33,120 | 약 $7.4 |

앵커는 반복을 1회로 줄인다(`--anchor-k`, 기본 1). 반복의 목적이 자기일관성
추정인데 앵커는 축소 일정에서 자기일관성을 쓰지 않기로 했으므로(설계서
7.4절), temp0 한 번이면 난이도 기여와 천장 확인이라는 앵커의 역할이
충족된다. 앵커 비용이 $12.1에서 $3.4로 준다.
