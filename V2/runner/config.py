...
MAIN_DESIGN = {
    ...
    "days": 20,                # 2026-09-06에 14에서 20으로 올렸다. 미국 팔의
                               # 피크 정의가 평일 한정(Anthropic 공표 8AM-2PM ET)
                               # 이라 14 완주일이면 평일이 10일뿐이고 피크 방문이
                               # 6.7회로 떨어진다. 20 완주일이면 평일 14일,
                               # 피크 방문 9.3회가 되어 선언 커버리지 0.15를
                               # 세 지표 모두에서 지탱한다(설계서 7.5절).
    "k": 3,                    # 자기일관성용 반복. 7.5절이 고른 값이다.
                               # 2026-09-03 실측: 은행 300과 일정이 고정이므로
                               # k마다 도달 가능한 tau 커버리지가 하나로 정해진다.
                               # k=2는 0.145, k=3은 0.160, k=4는 0.167, k=5는 0.170.
                               # 증분이 0.015, 0.007, 0.003으로 k=3에서 꺾인다.
